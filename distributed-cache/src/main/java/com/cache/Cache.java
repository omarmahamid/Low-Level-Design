...
import com.cache.events.*;
import com.cache.models.*;
...
public class Cache<KEY, VALUE> {
    ...
    public Cache(final int maximumSize,
        ...
        this(maximumSize, expiryTime, fetchAlgorithm, dataSource, new ConcurrentHashMap<>(), createPool(poolSize),
                new ConcurrentSkipListMap<>(), new CopyOnWriteArrayList<>(), new ConcurrentSkipListMap<>((first, second) -> {
        ...
        final CompletableFuture[] eagerLoading = keysToEagerlyLoad.stream()
    ...
    public Cache(final int maximumSize,
                    ...
                    final FetchAlgorithm fetchAlgorithm,
                    final DataSource<KEY, VALUE> dataSource,
                    final Map<KEY, CompletionStage<Record<KEY, VALUE>>> cache,
                    final ExecutorService[] executorPool,
                    final ConcurrentSkipListMap<Long, List<KEY>> expiryQueue,
                    final List<Event<KEY, VALUE>> eventQueue,
                    final ConcurrentSkipListMap<AccessDetails, List<KEY>> priorityQueue,
                    final Timer timer) {
    ...
    private static ExecutorService[] createPool(int poolSize){
        ExecutorService[] executorService = new ExecutorService[poolSize];
    ...
    private CompletionStage<Record<KEY, VALUE>> addToCache(final KEY key, final CompletionStage<VALUE> valueFuture) {
        manageEntries();
        final CompletionStage<Record<KEY, VALUE>> recordFuture = valueFuture.thenApply(value -> {
